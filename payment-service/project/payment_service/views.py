    ...
    def post(self, request):
            ...
            line_items.append(
                {
                    "price_data": {
                        "currency": "inr",
                        "product_data": {"name": each["product_details"]["name"]},
                        "unit_amount": int(each["product_details"]["price"] * 100),
                    },
                    "quantity": each["quantity"],
                }
            )
            total_amount += each["product_details"]["price"] * each["quantity"]
        ...
        try:
            session = stripe.checkout.Session.create(
                ...
                mode="payment",
                success_url=f"{settings.FRONTEND_URL}/ordersuccess/?session_id={{CHECKOUT_SESSION_ID}}",
